...
class Paging {
    ...
    public function __construct($db, $q='', $page_var='p')
    {
        ...
        if(isset($_GET[$this->page_var]) && $_GET[$this->page_var] == 'all')
        {
            $this->page_size = '';
        }
    }
    ...
    public function get_page($q='')
    {
        ...
        if (!empty($this->page_size) && $this->page_size !== 0) $this->total_pages = ceil($this->total_rows/$this->page_size);
    ...
    private function get_page_link($page, $text='' , $cl='')
    {
        $class = '';
        if(!empty($cl))
        {
        ...
        if ($page != $this->cur_page)
        {
            ...
            $reg = '/((&|^)'.$this->page_var.'=)[^&#]*/';
            $url = '?'.( preg_match( $reg, $uri ) ? preg_replace($reg, '${1}'.$page, $uri) : ( $uri ? $uri.'&' : '' ).$this->page_var.'='.$page);
            return '<li><a href="'.$url.'" ' .$class. '>'.$text.'</a></li>';
        }
        return '<li><a class="active">'.$text.'</a></li>';
    }

    public function get_allpage_link()
    {
        if(!empty($this->page_size))
        {
            ...
            return '<a class="button" href="'.$url.'?p=all'.'">Все</a>';
        }
    }
...
//        <span class="text">' .$arr_text['pages']. ':</span>
//	<ul>';
//$content .= $_PAGING->get_prev_page_link();
//$content .= $_PAGING->get_page_links();
//$content .= $_PAGING->get_next_page_link();
//$content .= '</ul>
//        </div>';
?>